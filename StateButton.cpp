// StateButton.cpp : implementation file
//
...
void CStateButton::OnLButtonDown(UINT nFlags, CPoint point) 
{
	if(m_id > 0)
...
void CStateButton::OnMouseMove(UINT nFlags, CPoint point) 
{
	if (!m_tracking)
	{
		TRACKMOUSEEVENT t = 
...
void CStateButton::OnLButtonUp(UINT nFlags, CPoint point) 
{
	if(m_id > 0)
...
LRESULT CStateButton::OnMouseLeave(WPARAM, LPARAM)
{
    m_tracking = false;
    Invalidate();
	return 0;
...
BOOL CStateButton::PreTranslateMessage(MSG* pMsg) 
{
	InitToolTip();
...
void CStateButton::SetSkin(UINT normal, UINT down, UINT over, UCHAR drawmode)
{
...
void CStateButton::DrawBitmap(CDC *dc, HBITMAP hbmp, RECT r, UCHAR DrawMode)
{
...
void CStateButton::PreSubclassWindow() 
{
	CButton::PreSubclassWindow();
	ModifyStyle(0, BS_OWNERDRAW);
}

void CStateButton::SetDown()
{
	m_bBuDown = true;
	Invalidate(FALSE);
}

void CStateButton::SetUp()
{
	m_bBuDown = false;
	Invalidate(FALSE);
}

void CStateButton::SetText(const char* sText,UCHAR left)
{
	if(sText!=NULL)
	{
		m_nTextLeft = left;
...
void CStateButton::EnableBalloonTooltip()
{
	m_dwToolTipStyle |= TTS_BALLOON;
}
void CStateButton::SetTooltipText(LPCTSTR lpszText, BOOL bActivate)
{
