...
LRESULT CControlButton::OnMouseLeave(WPARAM, LPARAM)
{
    m_tracking = false;
    Invalidate();
	return 0;
...
void CControlButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    if ((m_bBuDown) && (::GetCapture() == m_hWnd) && (::GetCursorPos(&mouse_position)))
	{
...
void CControlButton::SetSkin(UINT normal, UINT down, UINT over, UCHAR drawmode)
{
...
void CControlButton::DrawBitmap(CDC *dc, HBITMAP hbmp, RECT r, UCHAR DrawMode)
{
...
void CControlButton::SetDown()
{
	m_bBuDown = true;
	SetState(TRUE);
}

void CControlButton::SetUp()
{
	m_bBuDown = false;
	SetState(FALSE);
}

void CControlButton::SetText(const char* sText,UCHAR left)
{
	if(sText!=NULL)
	{
		m_nTextLeft = left;
